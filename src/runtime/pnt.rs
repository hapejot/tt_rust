...
impl Receiver for PointMetaReceiver {
    fn receive_message(&self, selector: &'static str, args: &[Rc<dyn Receiver>]) -> Rc<dyn Receiver> {
        match selector {
            "x:y:" => {
    ...
    fn receive_message(&self, selector: &'static str, args: &[Rc<dyn Receiver>]) -> Rc<dyn Receiver> {
        match selector {
            ...
            "+" => {
                ...
                let x = arg.receive_message("x", &[]).as_int().unwrap();
                let y = arg.receive_message("y", &[]).as_int().unwrap();
